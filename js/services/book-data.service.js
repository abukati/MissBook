...
const BOOK_API_KEY = 'AIzaSyC8ISivOVreS6ZDoVYNX2Ne8E0eQNPQi68'
...
function fetchBooks(searchTerm) {
  return axios.get(`https://www.googleapis.com/books/v1/volumes?printType=books&q=${searchTerm}&KEY=${BOOK_API_KEY}`)
    .then(res => res.data.items)
}

function addGoogleBook(book) {
  let newBook = _createBook(book)
  storageService.post(STORAGE_KEY, newBook)
}
...
function _createBook(book) {
  const newBook = {
    ...
    subtitle: book.volumeInfo.subtitle || 'Lorem, ipsum dolor sit amet consectetur adipisicing elit.',
    authors: book.volumeInfo.authors || [],
    publishedDate: book.volumeInfo.publishedDate,
    description: book.volumeInfo.description || utilService.getLorem(),
    pageCount: book.volumeInfo.pageCount,
    categories: book.volumeInfo.categories || ['general'],
    thumbnail: book.volumeInfo.imageLinks.thumbnail,
    language: book.volumeInfo.language,
    listPrice: {
      amount: utilService.getRandInt(),
      currencyCode: 'USD',
      isOnSale: false
    }
  }
...
function _saveBooks(data) {
  let books = utilService.loadFromStorage(STORAGE_KEY)
  if (!books || !books.length) {
    books = data
    utilService.saveToStorage(STORAGE_KEY, data)
  }
  return books
}
